	...
	if (props) {
		...
		switch (fillType) {
			case 'solid':
				...
				break
			case 'gradient':
				if (typeof props === 'object' && 'gradientStops' in props && Array.isArray(props.gradientStops) && props.gradientStops.length > 0) {
					const angle = props.gradientAngle || 0
					outText += '<a:gradFill rot="' + angle + '">'
					outText += '<a:gsLst>'
					props.gradientStops.forEach(stop => {
						outText += `<a:gs pos="${stop.position}">`
						outText += `<a:srgbClr val="${stop.color}"/>`
						outText += '</a:gs>'
					})
					outText += '</a:gsLst>'
					outText += '<a:lin ang="' + angle + '" scaled="0"/>'
					outText += '</a:gradFill>'
				}
				break
